...
export class ThreeScene {
  ...
  constructor() {
    ...
    this.ambientLight = new THREE.AmbientLight(this.colorPalette.indigo, 0.1);
    this.hemisphereLight = new THREE.HemisphereLight(this.colorPalette.yellow, this.colorPalette.indigo, 0.3);
    this.sun = new THREE.DirectionalLight(this.colorPalette.white, 0.5);
  ...
  init() {
    ...
    DEBUG &&
      console.log(`[ThreeScene.init] Done. Added ${this.resTracker.disposableResources.length} disposable resources`);
  }
  ...
  private createPointLight(color: THREE.Color) {
    ...
    if (this.showPointLightOrbs) {
      light.add(new THREE.Mesh(new THREE.SphereBufferGeometry(5, 16, 8), new THREE.MeshBasicMaterial({ color })));
    }
  ...
  private animate(currTime = 0) {
    ...
    this.pointLightGroup.children.forEach((light, i) => {
      if (light instanceof THREE.PointLight) {
        switch (i % 3) {
          case 1:
            light.position.x = Math.sin(delta * 0.7) * 250;
            light.position.y = Math.cos(delta * 0.5) * 500;
            light.position.z = Math.cos(delta * 0.3) * 250;
            break;
          case 2:
            light.position.x = Math.cos(delta * 0.3) * 250;
            light.position.y = Math.sin(delta * 0.5) * 500;
            light.position.z = Math.sin(delta * 0.7) * 250;
            break;
          default:
            light.position.x = Math.sin(delta * 0.3) * 250;
            light.position.y = Math.cos(delta * 0.7) * 500;
            light.position.z = Math.sin(delta * 0.5) * 250;
        }
      }
    });
